$(function () {
    $("#orders").datagrid({
        url: "OrdersList.aspx?action=bindOrders",
        fit: true,
        fitColumns: true,//自适应
        rownumbers: true,
        // border: false,
    ...
    orders_tool = {
        ...
        edit: function () {
            ...
            if (rows.length > 1 || rows.length <= 0) {
                ...
                $("#orders_edit").dialog('open');
                //$('input[name="pTitle1"]').focus();
                this.row = rows[0];
                var ID = this.row.ID;
                /*跳转到后台绑定到编辑页面*/
                $.ajax({
                    url: 'OrdersList.aspx?action=edit',
                    data: { ID: ID },
                    type: 'post',
    ...
    $("#orders_edit").dialog({
        ...
        buttons: [{
            ...
            handler: function () {
                if (($("#orders_edit").form("validate"))) {
                    $.ajax({
                        url: 'OrdersList.aspx?action=editOrders',
                        type: 'post',
                        data: {
                            ID: orders_tool.row.ID,
                            oState: $("#oState1").val(),
                        },
                        success: function (data) {
